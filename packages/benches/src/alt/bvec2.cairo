//! Alternative implementations benchmarked against `bvec2`.
//!
//! Every function here is a candidate formulation of a `glam::bvec2::BVec2` method, including a
//! copy of the winner so that the comparison stays reproducible. The cheapest one (l2_gas first,
//! then steps) lives in the library; `gas/bvec2.snap` holds the numbers (`alt_*` rows).

use glam::bvec2::BVec2;

const PANIC: felt252 = 'BVec2: index out of bounds';
...
#[inline(always)]
pub fn bitmask_if_tree(v: BVec2) -> u32 {
    if v.y {
        if v.x {
            3
        } else {
...
#[inline(always)]
pub fn bitmask_if_tree_lsb(v: BVec2) -> u32 {
    if v.x {
        if v.y {
            3
        } else {
...
#[inline(always)]
pub fn bitmask_felt(v: BVec2) -> u32 {
    let f: felt252 = v.x.into() + v.y.into() * 2;
    f.try_into().unwrap()
}

/// `bitmask` as a sum of `u32` terms selected by one `if` per element.
#[inline(always)]
pub fn bitmask_if_add(v: BVec2) -> u32 {
    let x: u32 = if v.x {
        1
    } else {
        0
...
#[inline(always)]
pub fn bitmask_if_add_felt(v: BVec2) -> u32 {
...
#[inline(always)]
pub fn bitmask_match(v: BVec2) -> u32 {
    match (v.x, v.y) {
        (false, false) => 0,
        (true, false) => 1,
        (false, true) => 2,
...
#[inline(always)]
pub fn any_ne_false(v: BVec2) -> bool {
    v != BVec2 { x: false, y: false }
}

/// `any` as an if-chain.
#[inline(always)]
pub fn any_if_chain(v: BVec2) -> bool {
    if v.x {
        true
    } else {
        v.y
...
#[inline(always)]
pub fn all_short_circuit(v: BVec2) -> bool {
    v.x && v.y
}
...
/// library version).
#[inline(always)]
pub fn all_eager(v: BVec2) -> bool {
    v.x & v.y
}

/// `all` as a `felt252` sum compared to the dimension.
#[inline(always)]
pub fn all_felt_sum(v: BVec2) -> bool {
    v.x.into() + v.y.into() == 2_felt252
}

/// `all` as a `felt252` product compared to one.
#[inline(always)]
pub fn all_felt_product(v: BVec2) -> bool {
    v.x.into() * v.y.into() == 1_felt252
}

/// `all` as a `match` of the `felt252` sum minus the dimension against zero.
#[inline(always)]
pub fn all_felt_match(v: BVec2) -> bool {
    match v.x.into() + v.y.into() - 2 {
        0 => true,
        _ => false,
    }
}
...
#[inline(always)]
pub fn all_eq_true(v: BVec2) -> bool {
    v == BVec2 { x: true, y: true }
}

/// `all` as an if-chain.
#[inline(always)]
pub fn all_if_chain(v: BVec2) -> bool {
    if v.x {
        v.y
    } else {
        false
...
#[inline(always)]
pub fn test_match(v: BVec2, index: usize) -> bool {
    match index {
        0 => v.x,
        1 => v.y,
...
#[inline(always)]
pub fn test_if_chain(v: BVec2, index: usize) -> bool {
    if index == 0 {
        v.x
    } else if index == 1 {
        v.y
...
#[inline(always)]
pub fn test_span(v: BVec2, index: usize) -> bool {
    match [v.x, v.y].span().get(index) {
...
#[inline(always)]
pub fn test_bitmask(v: BVec2, index: usize) -> bool {
...
#[inline(never)]
pub fn test_match_noinline(v: BVec2, index: usize) -> bool {
    match index {
        0 => v.x,
        1 => v.y,
        _ => core::panic_with_felt252(PANIC),
...
#[inline(always)]
pub fn set_match(ref v: BVec2, index: usize, value: bool) {
    match index {
        0 => v.x = value,
        1 => v.y = value,
        _ => core::panic_with_felt252(PANIC),
...
#[inline(always)]
pub fn set_match_rebuild(ref v: BVec2, index: usize, value: bool) {
    v = match index {
        0 => BVec2 { x: value, y: v.y },
        1 => BVec2 { x: v.x, y: value },
        _ => core::panic_with_felt252(PANIC),
    };
}
...
#[inline(always)]
pub fn set_if_chain(ref v: BVec2, index: usize, value: bool) {
    if index == 0 {
        v.x = value;
    } else if index == 1 {
        v.y = value;
...
#[inline(never)]
pub fn set_match_noinline(ref v: BVec2, index: usize, value: bool) {
    match index {
        0 => v.x = value,
        1 => v.y = value,
        _ => core::panic_with_felt252(PANIC),
...
#[inline(always)]
pub fn bitand_short_circuit(a: BVec2, b: BVec2) -> BVec2 {
    BVec2 { x: a.x && b.x, y: a.y && b.y }
}

/// `BitOr` with the short-circuit `||` instead of the eager `|`.
#[inline(always)]
pub fn bitor_short_circuit(a: BVec2, b: BVec2) -> BVec2 {
    BVec2 { x: a.x || b.x, y: a.y || b.y }
}

/// `BitOr` as `a ^ b ^ (a & b)`.
#[inline(always)]
pub fn bitor_xor_and(a: BVec2, b: BVec2) -> BVec2 {
    BVec2 { x: a.x ^ b.x ^ (a.x & b.x), y: a.y ^ b.y ^ (a.y & b.y) }
}

/// `BitXor` with `!=` instead of `^`.
#[inline(always)]
pub fn bitxor_ne(a: BVec2, b: BVec2) -> BVec2 {
    BVec2 { x: a.x != b.x, y: a.y != b.y }
}

/// `Not` as `a ^ true`.
#[inline(always)]
pub fn not_xor_true(a: BVec2) -> BVec2 {
    BVec2 { x: a.x ^ true, y: a.y ^ true }
}
...
#[inline(always)]
pub fn not_if(a: BVec2) -> BVec2 {
    BVec2 { x: flip(a.x), y: flip(a.y) }
}
