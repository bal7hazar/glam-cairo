...
Three layers, as in docs/DESIGN.md section 5:
  ...
  * tables over pools of edge values (0, +-1 raw, +-1, MIN, MAX, fractions): the expected values
    ...
    floor / round-half-even / round-to-nearest rules as `fixed::fixed` and `fixed::wide`;
...
def div(a, b):
    """`Fixed / Fixed`: `round_half_even(a * 2^32 / b)`, like `f64 /`."""
    return _half_even(a << 32, b)
...
def recip(x):
    """`Fixed::recip`: `round_half_even(2^64 / x)`."""
    return _half_even(1 << 64, x)
...
def gen_tests(t):
    ...
    # `length_recip` is `1 / length` rounded half to even: it needs a length of at least 1 raw ULP
    # whose reciprocal fits the scalar range (`Fixed::recip` overflows for a raw length of 1 or 2).
